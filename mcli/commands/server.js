...
function servesource(moduloWrapper, config, args) {
...
async function serve(moduloWrapper, config, args) {
...
function doServe(moduloWrapper, config, args, isSrcServe=false) {
    ...
    const _server = moduloWrapper._app.listen(port, host, () => {
        ...
        if (isSrcServe) {
            ...
            servesource(moduloWrapper, config, args);
        }
        console.log('|%|--------------');
    });
...
module.exports = { serve, servesource };
