...
equivalent
4. (DONE) Fix Demo.js to be better on mobile
5. (DONE) Add Custom CPart + API example

## Repo maintenance

1. Important: Create fresh GitHub repo at modulo.js/modulo, so it can have a
clean history, be more lightly cloned (after fixing stable builds?)
2. Move ssg-template to separate repo, include as Netlify docs..?
...
- Fixed build / definition ordering system
    ...
    - Ideas on recording and building:
        - Generate the build file separately just based on hashes, and then put
          that in alphabetical order based on hash (or based on hint name, for
          neatness). That way, the build will remain much more consistently
          ordered, e.g. we'll get the same hashes.
        - However, embed in a <script> tag the invocations on every page
          separately, so not all pages will activate all the same
        - We can even then make a "master JS hash" that's just the hash keys in
          alphabetical order, then hashed. This might make "maximum" builds fix
          everything
- modulo-create
